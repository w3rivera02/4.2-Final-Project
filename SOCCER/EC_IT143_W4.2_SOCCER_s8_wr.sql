...
TRUNCATE TABLE dbo.tbl_euro_winners;

INSERT INTO dbo.tbl_euro_winners (winner)
SELECT winner
FROM dbo.vw_euro_winners;

/*****************************************************************************************************************
NAME:    EC_IT143_W4.2_SOCCER_s7_wr.sql
PURPOSE: Stored procedure to refresh dbo.tbl_euro_winners by truncating and inserting fresh data from dbo.vw_euro_winners.

MODIFICATION LOG:
Ver      Date        Author         Description
-----   ----------   ------------   -------------------------------------------------------------------------------
1.0     07/25/2025   WRIVERA        1. Created stored procedure sp_refresh_euro_winners for refreshing table data.

RUNTIME: 
Xm Xs

NOTES: 
Stored procedures and views are both stored inside the database, making them secure and backed up with the database.
This stored procedure enables easy, repeatable refresh of the winners table.

******************************************************************************************************************/

IF OBJECT_ID('dbo.sp_refresh_euro_winners', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_refresh_euro_winners;
GO

CREATE PROCEDURE dbo.sp_refresh_euro_winners
AS
BEGIN
    SET NOCOUNT ON;
    ...
    INSERT INTO dbo.tbl_euro_winners (winner)
    SELECT winner FROM dbo.vw_euro_winners;
    EXEC dbo.sp_refresh_euro_winners;
